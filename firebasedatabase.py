...
# === Setup Logging ===
logging.basicConfig(level=logging.ERROR, filename='error.log', filemode='a', format='%(asctime)s - %(message)s')
...
def register():
    ...
    if st.button("Daftar"):
        if len(username) < 5:
            st.warning("Username harus terdiri dari minimal 5 karakter.")
            return
        if len(password) < 8:
...
def main_app():
    ...
    # Batasi rentang data
    if (end_date - start_date).days > 3650:  # Maksimum 10 tahun
        st.warning("Rentang waktu maksimal adalah 10 tahun.")
        st.stop()
        ...
        if data.empty:
            ...
            with tab4:  # Data
                ...
                if st.button("Unduh Data CSV"):
                    ...
                    st.download_button(label="Download CSV", data=csv, file_name=f"{ticker}_data.csv", mime='text/csv')
